...
def get_triple_diff_lc_array(e, M, x0, x_arr):
    Grav_const = 6.67e-11
    M_sun = 1.989e30
    c_speed = 2.998e8
    sec_per_yr = 3.15576e7
...
def getEQU_ISOchange(X, M_BH):
    ...
    # Precompute an inverse lookup table for target = Lfunc(cosi) * cosi.
    # This replaces millions of bisection iterations in the inner loop.
    cosi_grid = np.linspace(-1, 1, 5000)
    E_grid = Efunc(cosi_grid)
    L_grid = Lfunc(cosi_grid)
    Lc_grid = L_grid * cosi_grid

    if np.all(np.diff(Lc_grid) > 0):
        Lc_inverse_grid = Lc_grid
        cosi_inverse_grid = cosi_grid
        use_inverse_grid = True
    elif np.all(np.diff(Lc_grid) < 0):
        Lc_inverse_grid = Lc_grid[::-1]
        cosi_inverse_grid = cosi_grid[::-1]
        use_inverse_grid = True
    else:
        use_inverse_grid = False
    ...
    for y in y_arr:
        ...
        if L_d > L_cap:
            ...
            for e in e_arr:
                ...
                triple_diff_ISO_X = []
                triple_diff_ISO_M = []
                triple_rates = get_triple_diff_lc_array(
                    e=e,
                    M=M_BH,
                    x0=L0_norm_sq,
                    x_arr=L_sq_arr,
                ...
                for i, L in enumerate(L_sq_arr):
                    ...
                    if target > Lpro:
                        ...
                        Lisco = Lretro
                        #print("below")
                    elif use_inverse_grid:
                        cosiisco = np.interp(target, Lc_inverse_grid, cosi_inverse_grid)
                        Eisco = np.interp(cosiisco, cosi_grid, E_grid)
                        Lisco = np.interp(cosiisco, cosi_grid, L_grid)
                    else:
                        # Fallback to the original bisection if the inverse table is unsafe.
                        cosi_min = -1
                        cosi_max = 1
                        it = 0
                        while it < 1000:
                            cosi_guess = (cosi_min + cosi_max ) / 2
                            Lc_guess = Lfunc(cosi_guess)*cosi_guess
                            if abs(target - Lc_guess) < 10**-5:
                                break
                            elif target < Lc_guess:
                                cosi_max = cosi_guess
                            elif target > Lc_guess:
                                cosi_min = cosi_guess
                            it += 1
                    ...
                    triple_rate = triple_rates[i]
                    N_dot_3_diff_X = triple_rate *(Lisco * cosiisco - 2*X*Eisco) / (2*M_BH)
                    N_dot_3_diff_M = triple_rate * (M_sun / 2) * E_y
