...
# This file is inside final_project/CuPy_optimize.
THIS_DIR = Path(__file__).resolve().parent

# final_project is one folder up from CuPy_optimize.
...
# Load the versions we want to compare.
...
cupy_time, cupy_dXdt, cupy_dMdt = time_equ_iso(cupy_diff, X, M_BH)
...
# Compare CuPy version to original.
cupy_dXdt_error = abs(original_dXdt - cupy_dXdt)
cupy_dMdt_error = abs(original_dMdt - cupy_dMdt)
cupy_dXdt_match = np.isclose(original_dXdt, cupy_dXdt, rtol=REL_TOL, atol=ABS_TOL)
cupy_dMdt_match = np.isclose(original_dMdt, cupy_dMdt, rtol=REL_TOL, atol=ABS_TOL)
...
print(f"CuPy:        {cupy_time:.6f} seconds ({original_time / cupy_time:.2f}x)")
print() 
...
print(f"CuPy dXdt error:       {cupy_dXdt_error:.12e} match={cupy_dXdt_match}")
print(f"CuPy dMdt error:       {cupy_dMdt_error:.12e} match={cupy_dMdt_match}")
